...
struct Client
{
    ...
    bool never_focus;
    bool decorated;
    bool decorated_ignore_motif_hint;
    bool urgent;
...
struct Rule
{
    ...
    /* Note: Do not make these a bool. They have three states:
     * -1 = unchanged, 1 = force $property, 0 = force non-$property */
    char floating;
    char decorated;
...
decorations_draw(struct Client *c, enum DecorationWindowLocation which)
{
    ...
    if (dec_has_title)
    {
        ...
        if (c->enforced_title != NULL)
        {
            draw_text(dec_pm, font, &font_color[state], x, y, w,
                      center_title, c->enforced_title);
        }
        else
        {
            draw_text(dec_pm, font, &font_color[state], x, y, w,
...
handle_focusin(XEvent *e)
{
     ...
     *
     * Note that we can only enforce this policy for managed clients.
     * Clients that use override_redirect can still steal focus. They
     * will very likely call XSetInputFocus(.., RevertToParent, ...), so
     * when they quit, focus reverts to the root window.
     *
     * The following code covers both cases (managed client tries to
     * steal focus and unmanaged client with stolen focus quits). It is
     * written in a more explicit form, so it's easier to wrap your head
     * around this.
     *
     * Once more in prose:
     *
     * - If is_something_focused() == false, then we'll focus our
     *   "nofocus" dummy window, because we're on an empty workspace. We
     *   received the event on the root window.
     *
     * - Otherwise, focus != NULL, so it's fine to access it. Now we
     *   check if ev->window != focus->win. This can be true in two
     *   cases:
     *
     *   - ev->window == root, means an unmanaged client does unknown
     *     stuff,
     *
     *   - ev->window == some other managed window, means stealing
     *     attempt.
     *
     *   In both cases, focus->win is the window that really should be
     *   focused, so go ahead and do that. */

    D fprintf(stderr, __NAME_WM__": FocusIn: reported on %lu, root is %lu\n",
              ev->window, root);

    if (!is_something_focused())
    {
        D fprintf(stderr, __NAME_WM__": FocusIn: nothing supposed to have focus\n");
        manage_xfocus(NULL);
    }
    else if (ev->window != focus->win)
    {
        D fprintf(stderr, __NAME_WM__": FocusIn: ev->window != focus->win\n");
        manage_xfocus(focus);
    }
    else
        D fprintf(stderr, __NAME_WM__": FocusIn: no action\n");
}
    ...
    if (XGetClassHint(dpy, c->win, &ch))
    {
        ...
        for (i = 0; i < sizeof rules / sizeof rules[0]; i++)
        {
                ...
                strncmp(insta, rules[i].instance, insta_s) == 0)
            {
                ...
                else if (rules[i].workspace == -2)
                    c->workspace = monitors[c->mon].active_workspace;

                if (rules[i].floating != -1)
                    c->floating = rules[i].floating;

                if (rules[i].decorated != -1)
                {
                    manage_set_decorations(c, rules[i].decorated);

                    /* Unlike flags like "floating", the "decorated"
                     * flag is a bit more strict. It is possible to call
                     * manage_set_decorations() again later on, but we
                     * want to ignore future requests by the client.
                     * Why? Because the user's rules say that the client
                     * should be (un)decorated. Period. That's what the
                     * user wants, so the client has no say in this. */
                    c->decorated_ignore_motif_hint = true;
                }

                if (rules[i].enforced_title != NULL)
                    c->enforced_title = rules[i].enforced_title;
...
manage_apply_size(struct Client *c)
{
    ...
    if ((!c->hidden && c->fullscreen) || !c->decorated)
    {
...
manage_motif_evaluate_hints(struct Client *c, bool rearrange)
{
    ...
    if (c->decorated_ignore_motif_hint)
        return;
...
manage_xfocus(struct Client *c)
{
    if (c)
    {
        ...
        if (!c->never_focus)
        {
             ...
             * NO". */
            D fprintf(stderr, __NAME_WM__": xfocus to %lu, client\n", c->win);
    ...
    {
        D fprintf(stderr, __NAME_WM__": xfocus to %lu, nofocus\n", nofocus);
...
setup(void)
{
    ...
    font = XftFontOpenName(dpy, screen, dec_font);
    if (!font)
    {
    ...
    /* We cannot use _NET_WORKAREA, because it uses a different model of
     * "workspace" (the usual "one big area spanning all monitors"
     * problem). So, we sadly have to introduce another custom Atom.
     *
     * It will tell the world about the area where client window content
     * can be expected. This is mainly useful for xpointerbarrier.
     *
     * Top, left, right, bottom. */
    insets[0] = wai.top + gap_pixels + dgeo.top_height;
    insets[1] = wai.left + gap_pixels + dgeo.left_width;
    insets[2] = wai.right + gap_pixels + dgeo.right_width;
    insets[3] = wai.bottom + gap_pixels + dgeo.bottom_height;
    XChangeProperty(dpy, root, XInternAtom(dpy, IPC_ATOM_INSETS, False),
                    XA_INTEGER, 32, PropModeReplace, (unsigned char *)insets,
                    sizeof insets / sizeof insets[0]);
